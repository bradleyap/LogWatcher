...
if len(currentQuery) < 1 or currentQuery.find("query") < 0:
...
if len(output) < 1:
	output = 'JSON.parse(\'{"workableData":{"blocks":0,"timelineData":[],"timelineHint":{},"keywordColors":[],"statistics":"Error: No data returned from Luminator.jar"}}\')'
...
if not qfile is None:
        qfile.write("the HTML : " + htmlString)
	qfile.write("the current query is: " + currentQuery)
	qfile.close()
